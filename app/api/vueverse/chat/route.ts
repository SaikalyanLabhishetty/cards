...
import { VUEVERSE_AGENT_SYSTEM_PROMPT } from "@/app/lib/vueverseBot";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const GEMINI_MODEL_NAME =
  process.env.VUEVERSE_GEMINI_MODEL ||
...
const MISTRAL_MODEL_NAME =
  process.env.VUEVERSE_MISTRAL_MODEL ||
...
const GEMINI_FUNCTION_DECLARATIONS = [
  {
    ...
    parameters: {
      ...
      properties: {
        target: {
          type: "STRING",
          enum: ["linkedin", "github", "home", "calendly"],
...
async function callMistral(
  ...
  const mistralMessages = [
    {
      role: "system",
      content: VUEVERSE_AGENT_SYSTEM_PROMPT.trim(),
    },
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Mistral network request failed";
    return {
      ok: false,
      provider: "mistral",
      status: 502,
...
export async function POST(request: Request) {
  const geminiApiKey =
    process.env.VUEVERSE_GEMINI_API_KEY ||
    process.env.GEMINI_API_KEY ||
    process.env.GOOGLE_API_KEY ||
    "";
  const mistralApiKey =
    process.env.VUEVERSE_MISTRAL_API_KEY || process.env.MISTRAL_API_KEY || "";
  ...
  if (!hasGeminiKey && !hasMistralKey) {
    return NextResponse.json(
      {
        error:
          "Missing API keys. Set VUEVERSE_GEMINI_API_KEY (or GEMINI_API_KEY) and/or VUEVERSE_MISTRAL_API_KEY.",
        debug: {
          hasGeminiKey,
          hasMistralKey,
        },
  ...
  if (hasGeminiKey) {
    ...
    if (geminiResult.ok) {
      if (
        !geminiResult.text &&
        !geminiResult.toolCalls.length &&
        geminiResult.blockedReason
      ) {
        return NextResponse.json({
  ...
  return NextResponse.json(
    {
      ...
      hint: "Set VUEVERSE_MISTRAL_API_KEY to enable fallback.",
      gemini: geminiFailure,
    },
